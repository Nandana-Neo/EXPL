...
int code_gen_IF(tnode* node, FILE* fp){
    int i = code_gen(node->left,fp);    // code_gen(E)
    int l1 = get_label();
    fprintf(fp,"JZ R%d, _L%d\n",i,l1);
    code_gen(node->middle, fp); // code_gen(S1)
    int l2 = -1;
    ...
    fprintf(fp,"_L%d:\n",l1);
    if(node->right){    // if else node
        code_gen(node->right, fp);  // code_gen(S2)
        fprintf(fp,"_L%d:\n",l2);
    }     
...
int code_gen_WHILE(tnode* node, FILE* fp){
    ...
    int i = code_gen(node->left,fp);   // code_gen(E)
    fprintf(fp,"JZ R%d, _L%d\n",i,l2);
    code_gen(node->right, fp);  // code_gen(S1)
    fprintf(fp,"JMP _L%d\n",l1);
    fprintf(fp, "_L%d:\n",l2);
